...
class _InteractiveParticlesState extends State<InteractiveParticles>
  ...
  void _onTick(Duration elapsed) {
    ...
    for (final p in _particles) {
      p.update(
          safeDt, cursor, size, _burstEnergy, cursorActive, elapsedSeconds);
    }
...
class _Particle {
  ...
  void update(
    ...
    final tx = homeX * size.width +
        math.sin(elapsedSeconds * freqX + phaseX) * driftAmp;
    final ty = homeY * size.height +
        math.cos(elapsedSeconds * freqY + phaseY) * driftAmp;
...
class _ParticlesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final p in _particles) {
      ...
      if (burstBoost > 0.3) {
        c = Color.lerp(
            accentColor, isDark ? Colors.white : color, burstBoost * 0.4)!;
  ...
  void _drawGlowDot(
    ...
    canvas.drawCircle(
      ...
      Paint()
        ..color = c
        ..maskFilter =
            MaskFilter.blur(BlurStyle.normal, isDark ? r * 0.85 : r * 0.4)
        ..style = PaintingStyle.fill,
    );
    canvas.drawCircle(
      pos,
      ...
      Paint()
        ..color = (isDark ? Colors.white : c)
            .withValues(alpha: c.a * (isDark ? 0.5 : 0.95))
        ..style = PaintingStyle.fill,
    );
  }
  ...
  void _drawSparkle(Canvas canvas, Offset pos, double size, Color c) {
    ...
    canvas.drawPath(
      ...
      Paint()
        ..color = c
        ..maskFilter =
            MaskFilter.blur(BlurStyle.normal, size * (isDark ? 0.4 : 0.2))
        ..style = PaintingStyle.fill,
    );
    canvas.drawCircle(
      pos,
      ...
      Paint()
        ..color = (isDark ? Colors.white : c)
            .withValues(alpha: c.a * (isDark ? 0.8 : 1.0)),
    );
  }
